...
--
-- Applied by src/migrate.rs inside a transaction, recorded in schema_migrations.
-- Do not edit after release; add a new numbered file instead.
--
-- Integer columns are BIGINT throughout: the Rust side is i64 everywhere, and the
-- postgres driver refuses to decode INT4 into i64. The data is small; the width is free.
...
CREATE TABLE type_chart_rule (
  attacking_type_id BIGINT NOT NULL REFERENCES type(id) ON DELETE CASCADE,
  defending_type_id BIGINT NOT NULL REFERENCES type(id) ON DELETE CASCADE,
  multiplier_pct    BIGINT NOT NULL CHECK (multiplier_pct IN (0,50,100,200)),
...
CREATE TABLE pokemon (
  id              BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  national_dex_no BIGINT NOT NULL,
  -- '' = base form, else 'alolan', 'mega-x', ... In the natural key so variants do
  -- not collide on dex number. Empty string not NULL: UNIQUE treats NULLs as
  -- distinct by default, so NULL would permit base Raichu twice.
...
CREATE TABLE variant (
  ...
  base_pokemon_id BIGINT NOT NULL    REFERENCES pokemon(id) ON DELETE RESTRICT,
  variant_kind_id BIGINT NOT NULL    REFERENCES variant_kind(id),
...
CREATE TABLE pokemon_stats (
  ...
  base_hp         BIGINT NOT NULL CHECK (base_hp         BETWEEN 1 AND 255),
  base_attack     BIGINT NOT NULL CHECK (base_attack     BETWEEN 1 AND 255),
  base_defense    BIGINT NOT NULL CHECK (base_defense    BETWEEN 1 AND 255),
  base_sp_attack  BIGINT NOT NULL CHECK (base_sp_attack  BETWEEN 1 AND 255),
  base_sp_defense BIGINT NOT NULL CHECK (base_sp_defense BETWEEN 1 AND 255),
  base_speed      BIGINT NOT NULL CHECK (base_speed      BETWEEN 1 AND 255),
  base_stat_total BIGINT GENERATED ALWAYS AS
...
CREATE TABLE move_data (
  id               BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  move_id          BIGINT NOT NULL REFERENCES move(id) ON DELETE CASCADE,
  regulation_id    BIGINT NOT NULL REFERENCES regulation(id) ON DELETE RESTRICT,
  ...
  accuracy         BIGINT CHECK (accuracy      IS NULL OR accuracy      BETWEEN 0 AND 100),
  pp               BIGINT CHECK (pp            IS NULL OR pp > 0),
  priority         BIGINT NOT NULL DEFAULT 0,
  secondary_effect TEXT,
  effect_chance    BIGINT CHECK (effect_chance IS NULL OR effect_chance BETWEEN 0 AND 100),
...
CREATE TABLE pokemon_type (
  pokemon_id               BIGINT NOT NULL REFERENCES pokemon(id) ON DELETE CASCADE,
  slot                     BIGINT NOT NULL CHECK (slot IN (1,2)),
  type_id                  BIGINT NOT NULL REFERENCES type(id),
...
-- Partial unique indexes rather than triggers: declarative, and unlike an
-- EXISTS-check trigger they hold under concurrent writers.
-- Known limit: only open rows are constrained, so two overlapping CLOSED windows are
-- not caught. Unreachable via normal ingest (you only ever close the open row and
-- open a new one at the current regulation).

CREATE UNIQUE INDEX pokemon_type_one_open_window
    ON pokemon_type (pokemon_id, slot)
    WHERE valid_to_regulation_id IS NULL;

CREATE UNIQUE INDEX pokemon_ability_one_open_window
    ON pokemon_ability (pokemon_id, slot)
    WHERE valid_to_regulation_id IS NULL;

CREATE UNIQUE INDEX pokemon_move_one_open_window
    ON pokemon_move (pokemon_id, move_id, learn_method, level)
    WHERE valid_to_regulation_id IS NULL;
